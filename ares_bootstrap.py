"""Windows UTF-8 bootstrap for Ares entry points.
...
should be imported at the very top of every Ares entry point
(``ares``, ``ares-agent``, ``ares-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import ares_bootstrap``
# (or ``from ares_bootstrap import apply_windows_utf8_bootstrap``) at
