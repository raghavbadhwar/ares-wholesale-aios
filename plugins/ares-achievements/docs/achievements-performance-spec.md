# Ares Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: ares-achievements plugin
...
### `GET /api/plugins/ares-achievements/achievements`
...
### `POST /api/plugins/ares-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/ares-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/ares-achievements/overview`
...
- Persist plugin data under:
  - `~/.ares/plugins/ares-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.ares/plugins/ares-achievements/scan_checkpoint.json`
...
- `~/.ares/plugins/ares-achievements/`
