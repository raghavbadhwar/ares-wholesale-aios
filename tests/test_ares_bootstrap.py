"""Tests for ares_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Ares entry point
(ares, ares-agent, ares-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Ares entry point imports ares_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported ares_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("ares_bootstrap", None)
    import ares_bootstrap  # noqa: WPS433
    return ares_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Ares CLI
...
class TestEntryPointsImportBootstrap:
    """Every Ares entry point must import ares_bootstrap as its
    ...
    # Entry points that invoke Ares as a process.  Each one must
    # import ares_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "ares_cli/main.py",   # ares CLI (console_script)
        "run_agent.py",          # ares-agent (console_script)
        "acp_adapter/entry.py",  # ares-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import ares_bootstrap' and that
        ...
        half-finished ``ares update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``ares_bootstrap`` as a top-level module) leaves ares
        ...
        """
        # Resolve relative to the ares-agent repo root.  Tests live
        # at tests/test_ares_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # ares start even when ares_bootstrap hasn't been
        ...
        assert first_import_name == "ares_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'ares_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import ares_bootstrap' line to be the first import."
        )
