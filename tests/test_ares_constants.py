"""Tests for ares_constants module."""
...
from ares_constants import (
    ...
    get_default_ares_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultAresRoot:
    """Tests for get_default_ares_root() — Docker/custom deployment awareness."""

    def test_no_ares_home_returns_native(self, tmp_path, monkeypatch):
        """When ARES_HOME is not set, returns ~/.ares."""
        monkeypatch.delenv("ARES_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_ares_root() == tmp_path / ".ares"

    def test_ares_home_is_native(self, tmp_path, monkeypatch):
        """When ARES_HOME = ~/.ares, returns ~/.ares."""
        native = tmp_path / ".ares"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARES_HOME", str(native))
        assert get_default_ares_root() == native

    def test_ares_home_is_profile(self, tmp_path, monkeypatch):
        """When ARES_HOME is a profile under ~/.ares, returns ~/.ares."""
        native = tmp_path / ".ares"
    ...
    def test_ares_home_is_docker(self, tmp_path, monkeypatch):
        """When ARES_HOME points outside ~/.ares (Docker), returns ARES_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARES_HOME", str(docker_home))
        assert get_default_ares_root() == docker_home

    def test_ares_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ARES_HOME outside ~/.ares is treated as the root."""
        custom = tmp_path / "my-ares-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARES_HOME", str(custom))
        assert get_default_ares_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ARES_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARES_HOME", str(profile))
        assert get_default_ares_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(ares_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(ares_constants, "_container_detected", True)
        assert is_container() is True
