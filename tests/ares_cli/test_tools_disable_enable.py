"""Tests for ares tools disable/enable/list command (backend)."""
...
class TestToolsDisableBuiltin:
    ...
    def test_disable_removes_toolset_from_platform(self):
        ...
        with patch("ares_cli.tools_config.load_config", return_value=config), \
             patch("ares_cli.tools_config.save_config") as mock_save:
...
class TestToolsDisableMcp:
    ...
    def test_disable_unknown_server_prints_error(self, capsys):
        config = {"mcp_servers": {}}
        with patch("ares_cli.tools_config.load_config", return_value=config), \
             patch("ares_cli.tools_config.save_config"):
            tools_disable_enable_command(
