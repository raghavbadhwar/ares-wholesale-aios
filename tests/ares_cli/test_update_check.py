"""Tests for the update check mechanism in ares_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("ares_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ARES_HOME", str(tmp_path))
    with patch("ares_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import ares_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "ares_cli" / "banner.py"
    ...
    monkeypatch.setenv("ARES_HOME", str(tmp_path))
    with patch("ares_cli.banner.subprocess.run") as mock_run:
        with patch("ares_cli.banner.check_via_pypi", return_value=0):
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when ARES_HOME has no git repo."""
    import ares_cli.banner as banner
    ...
    # Point ARES_HOME at a temp dir with no ares-agent/.git
    monkeypatch.setenv("ARES_HOME", str(tmp_path))
    with patch("ares_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from ares_cli.main import _invalidate_update_cache

    # Build a fake ~/.ares with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"ARES_HOME": str(default_home)}):
