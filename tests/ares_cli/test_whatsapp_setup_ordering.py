...
Before the fix, ``ares whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``ares gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ARES_HOME", str(ares))
...
def _env_value(ares_home: Path, key: str) -> str | None:
    env_file = ares_home / ".env"
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("ares_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``ares whatsapp`` with an existing paired session and
