"""Tests for the `ares proxy` subcommand and its upstream adapters."""
...
from ares_cli.proxy.adapters import ADAPTERS, get_adapter
from ares_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from ares_cli.proxy.adapters.nous_portal import NousPortalAdapter
...
def _write_auth_store(ares_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic ARES_HOME."""
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # ARES_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "ares_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("ARES_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="ares login nous"):
...
from ares_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("ARES_HOME", str(tmp_path))
    from ares_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from ares_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from ares_cli.proxy.cli import cmd_proxy_start
