"""Tests for the decomposer module + `ares kanban decompose` CLI surface.
...
from ares_cli import kanban as kanban_cli
...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("ares_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("ares_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch("ares_cli.profiles.get_active_profile_name", return_value=names[0] if names else "default"),
    ]
...
def test_decompose_unknown_assignee_falls_back_to_default(kanban_home):
    ...
    try:
        with patch.dict(
            ...
            patch(
                "ares_cli.kanban_decompose._load_config",
                return_value={
                    "kanban": {
