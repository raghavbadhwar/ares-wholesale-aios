"""Tests for the kanban CLI surface (ares_cli.kanban)."""
...
from ares_cli import kanban as kc
...
def test_kanban_bypasses_active_session_guard():
    from ares_cli.commands import should_bypass_active_session

    assert should_bypass_active_session("kanban")


def test_kanban_in_autocomplete_table():
    from ares_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_not_gateway_only():
    ...
    from ares_cli.commands import COMMAND_REGISTRY
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("ARES_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("ARES_KANBAN_BOARD") == "beta"
