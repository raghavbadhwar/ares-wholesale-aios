...
When ``ARES_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.ares/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.ares            (has its own auth.json fixture)
    * Profile     -> tmp_path/.ares/profiles/coder   (active, ARES_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ARES_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.ares as the "real home" — which is NOT equal
    # to the ARES_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    ares_home = tmp_path / "classic"
    ares_home.mkdir()
    monkeypatch.setenv("ARES_HOME", str(ares_home))

    _write(ares_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from ares_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ARES_HOME is set to a custom path that is NOT under
    # ~/.ares/profiles/ — get_default_ares_root() returns ARES_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from ares_cli.auth import read_credential_pool, write_credential_pool
