"""Tests for the Ares plugin system (ares_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<ares_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<ares_home>/plugins/``; we derive
    ``<ares_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ARES_HOME>/config.yaml.
        # Config is always read from ARES_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        ares_home_str = os.environ.get("ARES_HOME")
        if ares_home_str:
            ares_home = Path(ares_home_str)
        else:
            ares_home = base.parent
        ares_home.mkdir(parents=True, exist_ok=True)
        cfg_path = ares_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.ares/plugins/ are discovered."""
        plugins_dir = tmp_path / "ares_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("ARES_HOME", str(tmp_path / "ares_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ARES_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".ares" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under ares_plugins.<name>."""
        plugins_dir = tmp_path / "ares_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ARES_HOME", str(tmp_path / "ares_test"))

        # Clean up any prior namespace module
        sys.modules.pop("ares_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "ares_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="ares_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import ares_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("ares_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "ares_test" / "plugins"
        # Set ARES_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("ares_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("ares_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("ares_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("ares_cli.plugins.PluginContext.dispatch_tool.__module__", "ares_cli.plugins"):
...
class TestPluginDebugLogging:
    """ARES_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("ARES_PLUGINS_DEBUG", raising=False)
        from ares_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With ARES_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("ARES_PLUGINS_DEBUG", "1")
        from ares_cli import plugins as plugins_mod
