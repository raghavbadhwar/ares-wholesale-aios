"""Tests for the top-level `./ares` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./ares` should use `ares_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "ares"
    called = []

    fake_main_module = types.ModuleType("ares_cli.main")

    def fake_main():
        called.append("ares_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "ares_cli.main", fake_main_module)
