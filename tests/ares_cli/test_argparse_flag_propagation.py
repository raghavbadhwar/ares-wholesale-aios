...
subparser, placing the flag BEFORE the subcommand (e.g. 'ares --yolo chat')
...
def _build_parser():
    """Build the ares argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets ARES_YOLO_MODE regardless of flag position.
    ...
    def _clean_env(self):
        os.environ.pop("ARES_YOLO_MODE", None)
        yield
        os.environ.pop("ARES_YOLO_MODE", None)
    ...
    def test_yolo_before_chat_sets_env(self):
        ...
        assert os.environ.get("ARES_YOLO_MODE") == "1"
    ...
    def test_no_yolo_no_env(self):
        ...
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("ARES_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `ares gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `ares <argv>` must exit 0 (help) rather than
        failing with `unrecognized arguments`."""
        import subprocess
        result = subprocess.run(
            [sys.executable, "-m", "ares_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=15,
        )
