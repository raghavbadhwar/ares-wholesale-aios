"""Tests for ares_cli.copilot_auth — Copilot token validation and resolution."""
...
class TestResolveToken:
    ...
    def test_copilot_github_token_first_priority(self, monkeypatch):
        from ares_cli.copilot_auth import resolve_copilot_token
    ...
    def test_gh_cli_fallback(self, monkeypatch):
        ...
        with patch("ares_cli.copilot_auth._try_gh_cli_token", return_value="gho_from_cli"):
            token, source = resolve_copilot_token()
    ...
    def test_gh_cli_classic_pat_raises(self, monkeypatch):
        ...
        monkeypatch.delenv("GH_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        with patch("ares_cli.copilot_auth._try_gh_cli_token", return_value="ghp_classic"):
    ...
    def test_no_token_returns_empty(self, monkeypatch):
        ...
        monkeypatch.delenv("GH_TOKEN", raising=False)
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        with patch("ares_cli.copilot_auth._try_gh_cli_token", return_value=None):
            token, source = resolve_copilot_token()
        assert token == ""
        assert source == ""
...
class TestRequestHeaders:
    ...
    def test_default_headers_include_openai_intent(self):
        ...
        assert headers["User-Agent"] == "AresAgent/1.0"
...
class TestCopilotDefaultHeaders:
    ...
    def test_includes_openai_intent(self):
        from ares_cli.models import copilot_default_headers
...
class TestApiModeSelection:
    ...
    def test_gpt5_uses_responses(self):
        from ares_cli.models import _should_use_copilot_responses_api
