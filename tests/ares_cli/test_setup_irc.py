"""Tests for IRC gateway configuration via `ares setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Ares install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Ares install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#ares")
            monkeypatch.setenv("IRC_NICKNAME", "ares-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `ares setup gateway` experience with IRC present."""
