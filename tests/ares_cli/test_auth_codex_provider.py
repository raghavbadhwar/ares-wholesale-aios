"""Tests for Codex auth — tokens stored in Ares auth store (~/.ares/auth.json)."""
...
def _setup_ares_auth(ares_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Ares auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_ares_auth(ares_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ARES_HOME", str(ares_home))
    ...
    monkeypatch.setattr("ares_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    ares_home = tmp_path / "ares"
    _setup_ares_auth(ares_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ARES_HOME", str(ares_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Ares auth store, not ~/.codex/."""
    ares_home = tmp_path / "ares"
    codex_home = tmp_path / "codex-cli"
    ares_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "ares-at", "refresh_token": "ares-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Ares store
    assert not (codex_home / "auth.json").exists()

    # Ares auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "ares-at"


def test_resolve_returns_ares_auth_store_source(tmp_path, monkeypatch):
    ares_home = tmp_path / "ares"
    _setup_ares_auth(ares_home)
    monkeypatch.setenv("ARES_HOME", str(ares_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "ares_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "ares_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("ares_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("ares_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
