...
The notice prints the most recent curator run summary on `ares update`,
...
session start) so users normally never see the rename map. `ares update`
...
def test_prints_multiline_summary_with_rename_map(curator_env):
    ...
    summary = (
        ...
        "full report: ares curator status"
    )
    _set_state(
        curator_env["curator"],
        last_run_at=now,
