...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ARES_HOME with .container-mode file."""
    ares_home = tmp_path / ".ares"
    ares_home.mkdir()
    monkeypatch.setenv("ARES_HOME", str(ares_home))
    monkeypatch.delenv("ARES_DEV", raising=False)

    container_mode = ares_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=ares-agent\n"
        "exec_user=ares\n"
        "ares_bin=/data/current-package/bin/ares\n"
    )
    return ares_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("ares_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "ares-agent"
    assert info["exec_user"] == "ares"
    assert info["ares_bin"] == "/data/current-package/bin/ares"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("ares_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_ares_dev(container_env, monkeypatch):
    """Returns None when ARES_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ARES_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_ares_dev_zero(container_env, monkeypatch):
    """ARES_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ARES_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ares_home = Path(tmpdir) / ".ares"
        ares_home.mkdir()
        (ares_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("ares_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_ares_home": lambda: ares_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ARES_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=ares-custom\n"
        "exec_user=myuser\n"
        "ares_bin=/opt/ares/bin/ares\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "ares-custom"
    assert info["exec_user"] == "myuser"
    assert info["ares_bin"] == "/opt/ares/bin/ares"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from ares_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "ares"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "ares-agent" in cmd
    assert "/data/current-package/bin/ares" in cmd
    assert "chat" in cmd
