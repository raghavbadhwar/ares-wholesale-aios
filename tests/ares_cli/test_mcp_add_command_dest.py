"""Regression test: ``ares mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``ares_cli/_parser.py``).  The dispatcher in ``ares_cli/main.py``
...
result, ``ares mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the ares parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`ares mcp add foo --url ...` must keep args.command == "mcp".
