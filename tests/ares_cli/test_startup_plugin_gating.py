...
``ares_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``ares --help``, ``ares version``,
``ares logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``ares --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from ares_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ares", "--tui", "chat"], "chat"),
        (["ares", "-w", "logs"], "logs"),
        (["ares", "chat", "hello world"], "chat"),
        (["ares", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["ares", "-m", "gpt5", "chat"], "chat"),
        (["ares", "--model", "gpt5", "chat", "hi"], "chat"),
        (["ares", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["ares", "-z", "hello world"], None),
        (["ares", "-z", "hello", "chat"], "chat"),
        (["ares", "--model=gpt5", "chat"], "chat"),     # inline form
        (["ares", "--", "chat"], "chat"),               # -- terminator
        (["ares", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["ares", "some-plugin-cmd"], "some-plugin-cmd"),
        (["ares", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["ares"],                          # bare → chat
        ["ares", "--help"],                # top-level help
        ["ares", "-h"],
        ["ares", "version"],               # known built-in
        ["ares", "logs"],
        ["ares", "gateway", "run"],
        ["ares", "--tui"],
        ["ares", "-w", "--tui"],
        ["ares", "chat", "hi"],
        ["ares", "help"],                  # accepted built-in-ish
        ["ares", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["ares", "meet", "join"],          # potential google_meet plugin
        ["ares", "honcho", "status"],      # potential memory plugin
        ["ares", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``ares help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"ares_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
