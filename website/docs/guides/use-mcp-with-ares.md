---
sidebar_position: 6
title: "Use MCP with Ares"
description: "A practical guide to connecting MCP servers to Ares Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Ares

This guide shows how to actually use MCP with Ares Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Ares tool
- you want Ares to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Ares to internal APIs, databases, or company systems without modifying Ares core

Do not use MCP when:
- a built-in Ares tool already solves the job well
...
- Ares remains the agent
- MCP servers contribute tools
- Ares discovers those tools at startup or reload time
...
If you installed Ares with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Ares banner/status should show MCP integration when configured
- ask Ares what tools it has available
...
## WSL2: bridge Ares in WSL to Windows Chrome

This is the practical setup when:

- Ares runs inside WSL2
...
In this setup, Ares does **not** connect to Chrome directly. Instead:

- Ares runs in WSL
- Ares starts a local stdio MCP server
...
```text
Ares (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Ares stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Ares core browser transport
...
```bash
ares mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
ares mcp test chrome-devtools-win
```

Then start a fresh Ares session or run:
...
Once loaded, Ares can use the MCP-prefixed browser tools directly. For example:
...
If Ares runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Ares from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Ares from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Ares:
...
So Ares will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Ares to reason over a bounded workspace.
...
Now Ares can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Ares core.
...
Because Ares now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
