---
sidebar_position: 7
title: "Use SOUL.md with Ares"
description: "How to use SOUL.md to shape Ares Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Ares

`SOUL.md` is the **primary identity** for your Ares instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Ares to feel like the same assistant every time you talk to it — or if you want to replace the Ares persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Ares should be
- what Ares should avoid stylistically
- how Ares should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Ares is and how Ares speaks
...
Ares now uses only the global SOUL file for the current instance:

```text
~/.ares/SOUL.md
```

If you run Ares with a custom home directory, it becomes:
...
Ares automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Ares does not overwrite it
- if the file exists but is empty, Ares adds nothing from it to the prompt

## How Ares uses it

When Ares starts a session, it reads `SOUL.md` from `ARES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.

If SOUL.md is missing, empty, or cannot be loaded, Ares falls back to a built-in default identity.
...
That alone can noticeably change how Ares feels.
...
Ares already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Ares should sound.

# Avoid
What Ares should not do.

# Defaults
How Ares should behave when ambiguity appears.
```
...
Then restart Ares or start a new session.
...
### I edited SOUL.md but Ares still sounds the same

Check:
- you edited `~/.ares/SOUL.md` or `$ARES_HOME/SOUL.md`
...
### Ares is ignoring parts of my SOUL.md
