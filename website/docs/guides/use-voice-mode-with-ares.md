---
sidebar_position: 8
title: "Use Voice Mode with Ares"
description: "A practical guide to setting up and using Ares voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Ares
...
- you want Ares sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Ares.
...
## Step 1: make sure normal Ares works first
...
```bash
pip install "ares-agent[voice]"
```

### Messaging platforms

```bash
pip install "ares-agent[messaging]"
```

### Premium ElevenLabs TTS

```bash
pip install "ares-agent[tts-premium]"
```
...
Ares supports both local and cloud speech stacks.
...
### If you use `ares setup`

If you choose NeuTTS in the setup wizard, Ares checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Ares transcribes and responds
...
- asking Ares to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Ares loop.

## Tuning CLI behavior

### Silence threshold

If Ares starts/stops too aggressively, tune:
...
Ares stays a normal chat bot, but can speak replies.
...
- you want Ares to function like a portable research or ops assistant
...
Ares joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Ares detects speech boundaries
- transcripts are posted in the associated text channel
- Ares responds in text and audio
...
1. get text Ares working
2. install `ares-agent[voice]`
