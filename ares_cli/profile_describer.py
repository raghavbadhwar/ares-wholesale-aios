...
Used by ``ares profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``ares_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Ares Agent kanban board.
...
Rules:
  ...
  - Never write "Ares Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from ares_constants import get_ares_home  # type: ignore
            profile_dir = Path(get_ares_home())
        else:
