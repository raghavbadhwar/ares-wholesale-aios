"""``ares logs`` — view and filter Ares log files.
...
Usage examples::
    ...
    ares logs                    # last 50 lines of agent.log
    ares logs -f                 # follow agent.log in real time
    ares logs errors             # last 50 lines of errors.log
    ares logs gateway -n 100    # last 100 lines of gateway.log
    ares logs --level WARNING    # only WARNING+ lines
    ares logs --session abc123   # filter by session ID substring
    ares logs --component tools  # only tool-related lines
    ares logs --since 1h         # lines from the last hour
    ares logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_ares_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Ares runs — try 'ares chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_ares_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_ares_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_ares_home()}/logs/")
        return

    print(f"Log files in {display_ares_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'ares chat' to generate logs)")
