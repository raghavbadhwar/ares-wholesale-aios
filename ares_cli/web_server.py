"""
Ares Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m ares_cli.main web          # Start on http://127.0.0.1:9119
    python -m ares_cli.main web --port 8080
"""
...
try:
    ...
    # running `ares dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ARES_WEB_DIST"]) if "ARES_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Ares Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Ares-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``ares dashboard --tui``
# or ARES_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.ares/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_ares_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "ares-update": "ares-update.log",
}
...
def _spawn_ares_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``ares <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``ares_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "ares_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ARES_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``ares gateway restart`` in the background."""
    try:
        proc = _spawn_ares_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/ares/update")
async def update_ares():
    """Kick off ``ares update`` in the background."""
    try:
        proc = _spawn_ares_action(["update"], "ares-update")
    except Exception as exc:
        _log.exception("Failed to spawn ares update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Ares supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in ares_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options():
    ...
    """
    try:
        from ares_cli.inventory import build_models_payload, load_picker_context
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.ares/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``ares auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Ares resolves Anthropic creds in this order at runtime:
    1. ``~/.ares/.anthropic_oauth.json`` — Ares-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_ares_oauth_credentials,
            read_claude_code_credentials,
            _ARES_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_ares_oauth_credentials = None  # type: ignore
        _ARES_OAUTH_FILE = None  # type: ignore

    ares_creds = None
    if read_ares_oauth_credentials:
        try:
            ares_creds = read_ares_oauth_credentials()
        except Exception:
            ares_creds = None
    if ares_creds and ares_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "ares_pkce",
            "source_label": f"Ares PKCE ({_ARES_OAUTH_FILE})",
            "token_preview": _truncate_token(ares_creds.get("accessToken")),
            "expires_at": ares_creds.get("expiresAt"),
            "has_refresh_token": bool(ares_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Ares even
    when they also have a separate Ares-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "ares auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "ares auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "ares auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "ares auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "ares auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from ares_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("ares_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Ares-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _ARES_OAUTH_FILE
            if _ARES_OAUTH_FILE.exists():
                _ARES_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from ares_cli.auth import clear_provider_auth
...
#          → persists to ~/.ares/.anthropic_oauth.json AND credential pool
...
# Guarded so ares web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Ares file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``ares auth add anthropic``.
    """
    from agent.anthropic_adapter import _ARES_OAUTH_FILE
    payload = {
    ...
    _ARES_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _ARES_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ARES_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from ares_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = (
            os.getenv("ARES_CODEX_BASE_URL", "").strip().rstrip("/")
...
_CHAT_ATTACHMENT_TEXT_LIMIT = 80_000
_CHAT_FILE_RE = re.compile(
    r"(?P<path>(?:~|/Users/raghav|/Volumes/RAGHAV2|/private/tmp|/tmp)[^\s`'\"<>)]+)"
)
_CHAT_FILE_KEY_HINTS = {
    "path",
    "file",
    "file_path",
    "filepath",
...
def _chat_session_segment(session_id: str | None) -> str:
    raw = (session_id or "draft").strip() or "draft"
    cleaned = re.sub(r"[^A-Za-z0-9_.-]+", "-", raw).strip(".-")
    return cleaned[:80] or "draft"


def _chat_upload_dir(session_id: str | None) -> Path:
    path = get_ares_home() / "chat_uploads" / _chat_session_segment(session_id)
...
def _chat_allowed_roots() -> list[Path]:
    roots = [PROJECT_ROOT, get_ares_home()]
    try:
...
def _resolve_chat_file(path: str) -> Path:
    raw = (path or "").strip()
    if not raw:
    ...
    if not resolved.is_file():
        raise HTTPException(status_code=404, detail="File not found")
    for root in _chat_allowed_roots():
        try:
            resolved.relative_to(root)
            return resolved
        except ValueError:
            continue
    raise HTTPException(status_code=403, detail="File is outside the allowed Ares/workspace roots")


def _chat_file_payload(path: Path, *, kind: str = "file") -> dict[str, Any]:
    try:
        stat = path.stat()
    except OSError:
        stat = None
    ...
    return {
        "name": path.name,
        "path": str(path),
        "url": f"/api/chat/files?path={urllib.parse.quote(str(path))}",
...
def _read_chat_attachment_excerpt(path: Path) -> str:
    try:
        data = path.read_bytes()[:_CHAT_ATTACHMENT_TEXT_LIMIT]
    ...
    for encoding in ("utf-8", "utf-16", "latin-1"):
        try:
            ...
            text = ""
    if not text:
        return "[Binary file uploaded. Use the file path with tools if inspection is needed.]"
    if path.stat().st_size > len(data):
        text += "\n\n[Attachment preview truncated.]"
    return text


def _format_chat_attachments(attachments: list[dict[str, Any]]) -> str:
    if not attachments:
        return ""
    parts = ["", "[Files attached in the Ares web chat]"]
    for index, item in enumerate(attachments, 1):
        try:
            path = _resolve_chat_file(str(item.get("path") or ""))
        except HTTPException as exc:
            parts.append(f"{index}. {item.get('name') or 'attachment'}: unavailable ({exc.detail})")
            continue
        payload = _chat_file_payload(path, kind="attachment")
        parts.append(
            f"{index}. {payload['name']} ({payload['type']}, {payload.get('size') or 0} bytes)\n"
            f"Path: {payload['path']}\n"
            "Preview:\n"
            f"{_read_chat_attachment_excerpt(path)}"
        )
    return "\n\n".join(parts)


def _collect_chat_file_paths(value: Any, found: set[str]) -> None:
    if isinstance(value, dict):
        for key, nested in value.items():
            key_l = str(key).lower()
            if key_l in _CHAT_FILE_KEY_HINTS or "path" in key_l or "file" in key_l:
                _collect_chat_file_paths(nested, found)
    ...
    if isinstance(value, str):
        for match in _CHAT_FILE_RE.finditer(value):
            candidate = match.group("path").rstrip(".,;:]}")
            found.add(candidate)


def _extract_chat_files(messages: list[dict[str, Any]], response_text: str = "") -> list[dict[str, Any]]:
    candidates: set[str] = set()
    _collect_chat_file_paths(response_text, candidates)
    for msg in messages[-24:]:
        ...
        if isinstance(content, str):
            try:
                ...
                decoded = content
            _collect_chat_file_paths(decoded, candidates)
        elif isinstance(content, (dict, list, tuple)):
            _collect_chat_file_paths(content, candidates)
        _collect_chat_file_paths(msg.get("tool_calls"), candidates)

    files: list[dict[str, Any]] = []
    seen: set[str] = set()
    for raw in sorted(candidates):
        try:
            path = _resolve_chat_file(raw)
        except HTTPException:
            continue
        key = str(path)
        if key in seen:
            continue
        seen.add(key)
        files.append(_chat_file_payload(path, kind="created"))
        if len(files) >= 12:
            break
    return files


def _dashboard_slash_help() -> str:
    return (
        "Available Ares web chat commands:\n\n"
        "- `/new` starts a fresh chat.\n"
        "- `/clear` clears the visible thread.\n"
        "- `/history` opens conversation history.\n"
        "- `/skills` shows enabled skills you can invoke.\n"
        "- `/memory` shows whether memory is enabled.\n"
        "- `/<skill-name> your task` invokes an installed skill with your instruction.\n"
        "- Attach files with the paperclip button; Ares receives the file path and a text preview."
    )


def _handle_dashboard_slash(user_message: str, session_id: str) -> tuple[bool, str]:
    """Return (handled_without_agent, message_or_rewritten_prompt)."""
    text = user_message.strip()
    if not text.startswith("/"):
        return False, user_message
    command, _, rest = text[1:].partition(" ")
    command_l = command.strip().lower()
    if command_l in {"help", "commands", "?"}:
        return True, _dashboard_slash_help()
    if command_l == "memory":
        return True, "Memory is enabled for this Ares web chat. Session history is persisted and resumed through the dashboard session store."
    if command_l in {"skills", "skill"} and not rest.strip():
        try:
            from agent.skill_commands import get_skill_commands
            skills = sorted(get_skill_commands().items(), key=lambda item: item[0])[:80]
            lines = ["Enabled skill commands:"]
            for key, info in skills:
                desc = str(info.get("description") or "").strip()
                lines.append(f"- `{key}` — {desc[:100]}")
            return True, "\n".join(lines) if len(lines) > 1 else "No enabled skill commands found."
        except Exception as exc:
            return True, f"Could not load skills: {exc}"
    try:
        from agent.skill_commands import build_skill_invocation_message, resolve_skill_command_key
        skill_key = resolve_skill_command_key(command_l)
        if skill_key:
            expanded = build_skill_invocation_message(
                skill_key,
                user_instruction=rest.strip(),
                task_id=session_id,
                runtime_note="Invoked from the Ares web chat command menu.",
            )
            if expanded:
                return False, expanded
    except Exception as exc:
        return True, f"Could not invoke skill `/{command_l}`: {exc}"
    command_prompts = {
        "validate-inputs": "Run the Ares validate-inputs workflow for the selected client and summarize blockers, parseable files, and the next action.",
        "daily-brief": "Generate the current daily business brief with approvals, overdue invoices, low stock, and owner actions.",
        "stock-radar": "Run stock radar and list low-stock SKUs, suggested reorder actions, and risks.",
        "overdue-invoices": "Find overdue invoices, prioritize follow-ups, and draft concise collection messages.",
    }
    if command_l in command_prompts:
        extra = rest.strip()
        return False, f"{command_prompts[command_l]} {extra}".strip()
    return False, user_message


@app.post("/api/chat/uploads")
async def upload_chat_file(request: Request, file: UploadFile = File(...), session_id: str = ""):
    _require_token(request)
    data = await file.read()
    if len(data) > _CHAT_UPLOAD_MAX_BYTES:
        raise HTTPException(status_code=413, detail="File is larger than the 25 MB chat upload limit")
    target_dir = _chat_upload_dir(session_id)
    base_name = _chat_safe_filename(file.filename or "attachment")
    ...
    return _chat_file_payload(target, kind="attachment")


@app.get("/api/chat/files")
async def get_chat_file(request: Request, path: str, download: bool = False):
    _require_token(request)
    target = _resolve_chat_file(path)
    media_type = mimetypes.guess_type(target.name)[0] or "application/octet-stream"
    disposition = "attachment" if download else "inline"
    return FileResponse(target, media_type=media_type, filename=target.name, content_disposition_type=disposition)


# ---------------------------------------------------------------------------
# /api/chat — WebSocket-based chat with the Ares AI agent.
#
# Protocol (JSON frames):
#   Client → Server:  { "message": "user text", "session_id": "optional-uuid" }
#   Server → Client:  { "type": "token",   "content": "chunk text" }
#                     { "type": "tool",    "name": "...", "status": "running|done" }
#                     { "type": "done",    "message": "full response", "session_id": "..." }
#                     { "type": "error",   "message": "error description" }
#                     { "type": "welcome", "session_id": "...", "model": "..." }
# ---------------------------------------------------------------------------


@app.websocket("/api/chat")
async def chat_ws(ws: WebSocket) -> None:
    """WebSocket chat endpoint for the Ares dashboard chat interface.

    Auth: ``?token=<session_token>`` query param (same as /api/pty).
    No TUI required — works with any dashboard launch mode.
    """
    ...
    # One AIAgent instance per active WebSocket conversation.  The client can
    # switch session_id values to resume older history without opening a new tab.
    import asyncio
    ...
    try:
        ...
        def _make_agent(target_session_id: str, restored_count: int = 0):
            instance = AIAgent(
                model=model,
            ...
            # Resumed turns pass the old history into run_conversation(); keep
            # this offset aligned so any explicit shutdown/final flush does not
            # replay already persisted rows.
            instance._last_flushed_db_idx = restored_count
            return instance

        session_id = _new_session_id()
        agent = _make_agent(session_id)

        # Send welcome frame so the UI knows the connection is live
        model_label = getattr(agent, "model", model) or model or "default"
        await ws.send_json({
            "type": "welcome",
            "session_id": session_id,
            "model": model_label,
        ...
        _log.warning("chat_ws: agent init failed: %s", exc)
        await ws.send_json({"type": "error", "message": f"Agent init failed: {exc}"})
        await ws.close()
        return
    ...
    async def _run_agent(user_message: str) -> str:
        """Run one stateful turn in a thread so the event loop stays responsive."""
        nonlocal conversation_history, session_id
        ...
        return await loop.run_in_executor(None, _turn)

    async def _bind_session(requested_session_id: str | None, *, force_new: bool = False) -> None:
        """Switch the live agent to a new or previously persisted session."""
        nonlocal agent, session_id, conversation_history
        if force_new:
            session_id = _new_session_id()
            conversation_history = []
            agent = _make_agent(session_id)
            await ws.send_json({"type": "session", "session_id": session_id, "history_count": 0})
            return
        ...
        if not resolved:
            await ws.send_json({"type": "error", "message": f"Session not found: {requested}"})
            return

        resume_id = session_db.resolve_resume_session_id(resolved) or resolved
        restored = session_db.get_messages_as_conversation(resume_id)
        try:
            session_db.reopen_session(resume_id)
        except Exception:
            pass
        session_id = resume_id
        conversation_history = restored
        agent = _make_agent(session_id, restored_count=len(restored))
        await ws.send_json({
            "type": "session",
            "session_id": session_id,
    ...
    try:
        while True:
            ...
            if not raw_user_message and not attachments:
                continue

            await _bind_session(
                str(payload.get("session_id") or ""),
                force_new=bool(payload.get("new_session")),
            )

            handled_slash, user_message = _handle_dashboard_slash(raw_user_message, session_id)
            attachment_note = _format_chat_attachments(attachments)
            if attachment_note:
                user_message = (user_message or "Please review the attached files.").rstrip() + "\n\n" + attachment_note

            if handled_slash and not attachment_note:
                await ws.send_json({
                    "type": "done",
            ...
            try:
                response = await _run_agent(user_message)
                files = _extract_chat_files(conversation_history, response)
                await ws.send_json({
                    "type": "done",
                    "message": response,
                    "session_id": session_id,
                ...
                _log.warning("chat_ws: agent.chat error: %s", exc)
                await ws.send_json({"type": "error", "message": str(exc)})

    except Exception:
        # WebSocketDisconnect or any other error — just clean up
        pass
    finally:
        try:
            if agent is not None and conversation_history:
                agent.commit_memory_session(conversation_history)
        except Exception:
            pass
        try:
...
async def get_logs(
    ...
    from ares_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_ares_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from ares_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_ares_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from ares_cli import profiles as profiles_mod
    try:
...
def _profile_setup_command(name: str) -> str:
    ...
    return "ares setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from ares_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from ares_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from ares_cli.tools_config import (
...
#
# The endpoint spawns the same ``ares --tui`` binary the CLI uses, behind
...
try:
    from ares_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``ares --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via both ``ARES_TUI_RESUME`` and
    ``HERMES_TUI_RESUME`` env vars —
    matching what ``ares_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as both ``ARES_TUI_SIDECAR_URL`` and
    ``HERMES_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from ares_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ARES_PYTHON_SRC_ROOT", str(PROJECT_ROOT))
    env.setdefault("HERMES_PYTHON_SRC_ROOT", env["ARES_PYTHON_SRC_ROOT"])
    env.setdefault("ARES_PYTHON", sys.executable)
    env.setdefault("HERMES_PYTHON", env["ARES_PYTHON"])
    env.setdefault("NODE_ENV", "production")
    env.setdefault("ARES_SKIN", "ares")
    env.setdefault("HERMES_SKIN", env["ARES_SKIN"])
    ...
    env.setdefault("ARES_TUI_DISABLE_MOUSE", "1")
    env.setdefault("HERMES_TUI_DISABLE_MOUSE", env["ARES_TUI_DISABLE_MOUSE"])

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ARES_TUI_RESUME"] = resume
        env["HERMES_TUI_RESUME"] = resume

    if sidecar_url:
        env["ARES_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Ares inside WSL2 to use the dashboard's /chat "
...
# ARES_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/ares"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/ares/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /ares`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ARES_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/ares``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__ARES_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f'window.__HERMES_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__ARES_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f"window.__HERMES_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__ARES_BASE_PATH__="{prefix}";'
    ...
    # under ``/ares`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Ares backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Ares Teal",         "description": "Classic dark teal — the canonical Ares look"},
    {"name": "default-large", "label": "Ares Teal (Large)", "description": "Ares Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.ares/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.ares/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_ares_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.ares/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as ares_cli.plugins):
    1. User plugins:    ~/.ares/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.ares/plugins/  (only if ARES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from ares_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_ares_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("ARES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".ares" / "plugins", "project"))
    ...
    for plugins_root, source in search_dirs:
        ...
        for child in sorted(plugins_root.iterdir()):
            ...
            try:
                ...
                continue
    if any(plugin.get("name") == "ares-achievements" for plugin in plugins):
        plugins = [plugin for plugin in plugins if plugin.get("name") != "hermes-achievements"]
    return plugins
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from ares_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_ares_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from ares_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from ares_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from ares_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from ares_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"ares_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Ares Web UI → http://{host}:{port}")
