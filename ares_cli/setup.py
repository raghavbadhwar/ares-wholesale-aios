"""
Interactive setup wizard for Ares Agent.
...
from ares_cli.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from utils import base_url_hostname
from ares_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://ares-agent.nousresearch.com/docs"
...
# display_ares_home imported lazily at call sites (stale-module safety during ares update)
...
from ares_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Ares Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Ares using environment variables or config commands:")
    print_info("  ares config set model.provider custom")
    print_info("  ares config set model.base_url http://localhost:8080/v1")
    print_info("  ares config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'ares setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from ares_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from ares_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'ares setup')")


def _print_setup_summary(config: dict, ares_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'ares setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from ares_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `ares tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from ares_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'ares setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'ares setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'ares setup terminal'"))
    ...
    # Spotify (OAuth via ares auth spotify — check auth.json, not env vars)
    try:
        from ares_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'ares setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {ares_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('ares setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('ares setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('ares setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('ares setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('ares setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('ares config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('ares config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('ares config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('ares', Colors.GREEN)}              Start chatting")
    print(f"   {color('ares gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('ares doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `ares tools` and `ares setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``ares model``)
    ...
    provider added to ``ares model`` is automatically available here.
    ...
    # Delegate to the shared ares model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from ares_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: ares model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from ares_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Ares can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Ares can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'ares setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``ares model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
        from ares_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.ares/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Ares auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via ares setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'ares setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Ares runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'ares-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'ares-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ARES_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ARES_MAX_ITERATIONS")
    ...
    print_info("  Run `ares setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ARES_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Ares delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `ares update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://ares-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ARES_HOME, and print
    ...
    """
    try:
        from ares_cli.slack_cli import _build_full_manifest
        from ares_constants import get_ares_home

        manifest = _build_full_manifest(
            bot_name="Ares",
            bot_description="Your Ares agent on Slack",
        )
        target = Path(get_ares_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `ares slack manifest --write` anytime to refresh after "
            "Ares adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Ares delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Ares delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  ares config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Ares to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from ares_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://ares-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://ares-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  ares config edit")
    print_info("   Open config in your editor:  ares config edit")
...
def setup_gateway(config: dict):
    ...
    from ares_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Ares from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'ares setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     ares config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_ares_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  ares gateway")
        else:
            from ares_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   ares gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   ares gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `ares setup tools` and `ares tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``ares_cli.auth`` — lists every supported
    ...
    """
    try:
        from ares_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from ares_cli.config
    ...
    if section_key == "model":
        ...
        from ares_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_ares migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_ares", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Ares.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Ares to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Ares at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Ares at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Ares at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Ares at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Ares equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Ares config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Ares.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Ares's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(ares_home: Path) -> bool:
    ...
    print_info("Ares can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: ares claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: ares claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Ares configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Ares config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Ares (use ares claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      ares setup           — full or quick (auto-detected)
      ares setup model     — just model/provider
      ares setup tts       — just text-to-speech
      ares setup terminal  — just terminal backend
      ares setup gateway   — just messaging platforms
      ares setup tools     — just tool configuration
      ares setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Ares Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Ares Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Ares Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, ares_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Ares configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'ares setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(ares_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Ares?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, ares_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {ares_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'ares config edit'")
    ...
    _print_setup_summary(config, ares_home)


def _run_first_time_quick_setup(config: dict, ares_home, is_existing: bool):
    ...
    the user can customize later via ``ares setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'ares setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    ares setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  ares setup gateway")
    print()

    _print_setup_summary(config, ares_home)


def _run_quick_setup(config: dict, ares_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'ares setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Ares to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'ares setup gateway'.")
