"""CLI subcommand: ``ares send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"ares send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"ares send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"ares send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"ares send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"ares send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `ares gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.ares/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_ares_env() -> None:
    """Populate ``os.environ`` from ``~/.ares/.env`` AND bridge top-level
    ...
    startup that ``ares send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.ares/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.ares/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``ares send``
    ...
    try:
        from ares_cli.config import _expand_env_vars
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.ares/.env and ~/.ares/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "ares send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  ares send --to telegram \"hello\"\n"
            "  ares send --to discord:#ops --file report.md\n"
            "  ares send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "ares send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `ares send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Ares "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.ares/.env + ~/.ares/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  ares send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | ares send --to telegram:-1001234567890\n"
            "  ares send --to discord:#ops --file /tmp/report.md\n"
            "  ares send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  ares send --list                  # all platforms\n"
            "  ares send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `ares send --list telegram`.",
    )
