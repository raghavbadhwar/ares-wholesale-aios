"""`ares checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.ares/checkpoints/``.  Actions:

    ares checkpoints               # same as `status`
    ares checkpoints status        # total size, project count, breakdown
    ares checkpoints list          # per-project checkpoint counts + workdir
    ares checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    ares checkpoints clear [-f]    # nuke the entire base (asks first)
    ares checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    ares checkpoints
    ares checkpoints prune --retention-days 3 --max-size-mb 200
    ares checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: ares checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``ares checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `ares checkpoints` → status
