...
on Japanese locales, etc.).  Ares's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``ares_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Ares is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ARES_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ARES_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Ares-managed Git install directories so
    ...
    # already-running shells don't see the change — which means ares
    ...
    # install without Ares-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Ares's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Ares's
    ``ares config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Ares (User env var in Windows Settings, or
    export in a PowerShell profile) and Ares picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Ares-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\ares\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``ares`` from right after install) retain their old PATH.

    Any subprocess Ares spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Ares-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "ares", "git", "cmd"),
        os.path.join(local_appdata, "ares", "git", "bin"),
        os.path.join(local_appdata, "ares", "git", "usr", "bin"),
        # Ares venv Scripts directory — host of the ares.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes ares, but harmless to include.
        os.path.join(local_appdata, "ares", "ares-agent", "venv", "Scripts"),
