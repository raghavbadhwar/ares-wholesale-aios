"""
Single source of truth for provider identity in Ares Agent.
...
2. **Ares overlays** — transport type, auth patterns, aggregator flags,
...
# -- Ares overlay ----------------------------------------------------------
# Ares-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class AresOverlay:
    """Ares-specific provider metadata layered on top of models.dev."""
...
ARES_OVERLAYS: Dict[str, AresOverlay] = {
    "openrouter": AresOverlay(
    ...
    "openai-codex": AresOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": AresOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": AresOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ARES_QWEN_BASE_URL",
    ),
    "google-gemini-cli": AresOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": AresOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": AresOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": AresOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": AresOverlay(
    ...
    "minimax-oauth": AresOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": AresOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": AresOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": AresOverlay(
        transport="openai_chat",
    ...
    "opencode": AresOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": AresOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "ares", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Ares overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Ares overlay
    ...
    overlay = ARES_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + ares extra
    ...
    if overlay is not None:
        # Ares-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ARES_OVERLAYS
    if provider == "bedrock":
