"""
Profile management for multiple isolated Ares instances.

Each profile is a fully independent ARES_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.ares/profiles/<name>/`` by default.

The "default" profile is ``~/.ares`` itself — backward compatible,
zero migration needed.

Usage::

    ares profile create coder          # fresh profile + bundled skills
    ares profile create coder --clone  # also copy config, .env, SOUL.md, skills
    ares profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    ares -p coder chat                 # or via flag
    ares profile use coder             # set as sticky default
    ares profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See ares_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `ares profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `ares
...
# `ares skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Ares infrastructure directories that only the default profile
         (``~/.ares``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_ares_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.ares) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "ares-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "ares", "default", "test", "tmp", "root", "sudo",
})

# Ares subcommands that cannot be used as profile names/aliases
_ARES_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the ares root, NOT to the current ARES_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ARES_HOME points outside
    ``~/.ares``, profiles live under ``ARES_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_ares_home() / "profiles"


def _get_default_ares_home() -> Path:
    """Return the default (pre-profile) ARES_HOME path.

    In standard deployments this is ``~/.ares``.
    In Docker/custom deployments where ARES_HOME is outside ``~/.ares``
    (e.g. ``/opt/data``), returns ARES_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_ares_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Ares installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ARES_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_ares_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, ares subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ARES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a ares subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``ares profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Ares config
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``ares update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.ares)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.ares/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `ares update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`ares profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `ares update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `ares profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ARES_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``ares profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ARES_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.ares).\n"
            "To remove everything, use: ares uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ARES_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ARES_HOME")
    try:
        os.environ["ARES_HOME"] = str(profile_dir)
        from ares_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        elif "ARES_HOME" in os.environ:
            del os.environ["ARES_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.ares/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: ares profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ARES_HOME.

    Returns ``"default"`` if ARES_HOME is not set or points to ``~/.ares``.
    Returns the profile name if ARES_HOME points into ``~/.ares/profiles/<name>``.
    Returns ``"custom"`` if ARES_HOME is set to an unrecognized path.
    """
    from ares_constants import get_ares_home
    ares_home = get_ares_home()
    resolved = ares_home.resolve()

    default_resolved = _get_default_ares_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.ares itself — its parent is ~/ and its
        # directory name is ".ares", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: ares profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.ares itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.ares). "
            "Specify a different name: ares profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="ares_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"ares.{old_name}"
    new_host = f"ares.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_ares_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ARES_HOME path string.

    Called early in the CLI entry point, before any ares modules
    are imported, to set the ARES_HOME environment variable.
    """
