"""CLI handlers for the ``ares proxy`` subcommand."""
...
from ares_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "ares proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        print(
            ...
            f"Run `ares login {adapter.name}` first.",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        f"Starting Ares proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Ares proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: ares proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``ares proxy <subcommand>``."""
    ...
    print(
        "ares proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  ares proxy start [--provider nous] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  ares proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  ares proxy providers\n"
