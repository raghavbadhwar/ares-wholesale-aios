...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m ares_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Ares_Gateway"
_TASK_DESCRIPTION = "Ares Agent Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    Default profile: ``Ares_Gateway``
    Named profile X: ``Ares_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during ares_cli boot.
    from ares_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\ares\\gateway-service\\<task_name>.cmd``
    (or ``<ARES_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Ares installs stay self-contained).
    """
    _assert_windows()
    from ares_cli.config import get_ares_home

    script_dir = Path(get_ares_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ARES_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m ares_cli.main [--profile X] gateway run --replace``
    ...
    lines.append(f'set "ARES_HOME={ares_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ARES_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports ares_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    prog_args = [python_path, "-m", "ares_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    ares_home = str(Path(get_ares_home()).resolve())
    profile_arg = _profile_arg(ares_home)

    content = _build_gateway_cmd_script(python_path, working_dir, ares_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "ares_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m ares_cli.main gateway run --replace``
    ...
    log_dir = Path(get_ares_home()) / "logs"
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from ares_cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from ares_cli.config import get_ares_home
        print(f"    type {Path(get_ares_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_ares_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from ares_cli.config import get_ares_home

    ares_home = Path(get_ares_home()).resolve()
    print()
    print("Next steps:")
    print("  ares gateway status                      # Check status")
    print(f"  type {ares_home}\\logs\\gateway.log       # View logs")
...
def stop() -> None:
    ...
    _assert_windows()
    from ares_cli.gateway import kill_gateway_processes
