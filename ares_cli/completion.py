"""Shell completion script generation for ares CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_ares_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Ares Agent bash completion
# Add to ~/.bashrc:
#   eval "$(ares completion bash)"

_ares_profiles() {{
    local profiles_dir="$HOME/.ares/profiles"
...
_ares_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_ares_profiles)" -- "$cur"))
        return
    fi
...
complete -F _ares_completion ares
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef ares
# Ares Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(ares completion zsh)"

_ares_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.ares/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.ares/profiles 2>/dev/null)}}")
    fi
...
_ares() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_ares_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'ares command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Ares Agent fish completion",
        "# Add to your config:",
        "#   ares completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __ares_profiles",
        "    echo default",
        "    if test -d $HOME/.ares/profiles",
        "        ls $HOME/.ares/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c ares -f -s p -l profile"
        " -d 'Profile name' -xa '(__ares_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__ares_profiles)' -d 'Profile name'"
                )
