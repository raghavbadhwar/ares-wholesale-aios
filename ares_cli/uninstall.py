"""
Ares Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.ares/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Ares PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing ares-agent or ares PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Ares Agent" comment and following line
                if '# Ares Agent' in line or '# ares-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('ares' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing ares
                if 'ares' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the ares wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "ares",
        Path("/usr/local/bin/ares"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains ares_cli reference)
                content = wrapper.read_text()
                if 'ares_cli' in content or 'ares-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``ares gateway run`` processes
    ...
    try:
        from ares_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from ares_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ARES_HOME`` and ``ARES_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\ares\git\cmd``, ``%LOCALAPPDATA%\ares\git\bin``,
#      ``%LOCALAPPDATA%\ares\git\usr\bin``, ``%LOCALAPPDATA%\ares\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\ares\git\`` and Node to
#      ``%LOCALAPPDATA%\ares\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``ares dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\ares\gateway-service\`` and sometimes
...
def _ares_path_markers(ares_home: Path) -> list[str]:
    """Path-entry substrings that identify Ares-owned User-PATH entries."""
    root = str(ares_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare ares-agent install dir.
    markers = [root + "\\ares-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ARES_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "ares".  We don't want to catch
    # unrelated entries like "cares-foo" or "ephermeral", so we look for
    # backslash-ares as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(ares_home: Path) -> list[str]:
    """Strip Ares-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _ares_path_markers(ares_home)
            kept: list[str] = []
...
def remove_ares_env_vars_windows() -> list[str]:
    """Delete ARES_HOME and ARES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ARES_HOME", "ARES_GIT_BASH_PATH"):
                try:
...
def _is_default_ares_home(ares_home: Path) -> bool:
    """Return True when ``ares_home`` points at the default (non-profile) root."""
    try:
        from ares_constants import get_default_ares_root
        return ares_home.resolve() == get_default_ares_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ARES_HOME directory.

    We shell out to ``ares -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ARES_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m ares_cli.main` so we don't depend on a `ares`
    #    wrapper that may be half-removed mid-uninstall.
    ares_invocation = [_sys.executable, "-m", "ares_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                ares_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ARES_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.ares/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.ares/ for future reinstall
    """
    project_root = get_project_root()
    ares_home = get_ares_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ARES_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_ares_home(ares_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Ares Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {ares_home / 'config.yaml'}")
    print(f"  Secrets: {ares_home / '.env'}")
    print(f"  Data:    {ares_home / 'cron/'}, {ares_home / 'sessions/'}, {ares_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ARES_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Ares data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Ares code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in ares_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\ares\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(ares_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Ares-owned PATH entries in User environment")

        log_info("Removing ARES_HOME / ARES_GIT_BASH_PATH User env vars...")
        removed_env = remove_ares_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Ares-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing ares command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.ares/, just remove the ares-agent subdir
            if ares_home in project_root.parents or project_root.parent == ares_home:
    ...
    #     under ARES_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(ares_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(ares_home)
    ...
    # 5. Optionally remove ~/.ares/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ARES_HOME dirs live under
        ...
        try:
            if ares_home.exists():
                shutil.rmtree(ares_home)
                log_success(f"Removed {ares_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {ares_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {ares_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/NousResearch/ares-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/ares-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Ares Agent! ⚕")
    print()
