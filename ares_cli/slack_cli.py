"""``ares slack ...`` CLI subcommands.

Today only ``ares slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ ares slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Ares. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Ares deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from ares_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Ares agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Ares in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``ares_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ARES_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Ares")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Ares"
    description = getattr(args, "description", None) or "Your Ares agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("ARES_HOME") or str(Path.home() / ".ares")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Ares app\n"
