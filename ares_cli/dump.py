"""
Dump command for ares CLI.

Outputs a compact, plain-text summary of the user's Ares setup
...
from ares_cli.config import get_ares_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``ares dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from ares_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(ares_home: Path) -> int:
    """Count installed skills."""
    skills_dir = ares_home / "skills"
...
def _cron_summary(ares_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = ares_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"ares_home:      {display_ares_home()}")
    ...
    toolsets = config.get("toolsets", ["ares-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(ares_home)}")
    lines.append(f"  skills:             {_count_skills(ares_home)}")
