...
Usage:
    ares                     # Interactive chat (default)
    ares chat                # Interactive chat
    ares gateway             # Run gateway in foreground
    ares gateway start       # Start gateway as service
    ares gateway stop        # Stop gateway service
    ares gateway status      # Show gateway status
    ares gateway install     # Install gateway service
    ares gateway uninstall   # Uninstall gateway service
    ares setup               # Interactive setup wizard
    ares logout              # Clear stored authentication
    ares status              # Show status of all components
    ares cron                # Manage cron jobs
    ares cron list           # List cron jobs
    ares cron status         # Check if cron scheduler is running
    ares doctor              # Check configuration and dependencies
    ares honcho setup                    # Configure Honcho AI memory integration
    ares honcho status                   # Show Honcho config and connection status
    ares honcho sessions                 # List directory → session name mappings
    ares honcho map <name>               # Map current directory to a session name
    ares honcho peer                     # Show peer names and dialectic settings
    ares honcho peer --user NAME         # Set user peer name
    ares honcho peer --ai NAME           # Set AI peer name
    ares honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    ares honcho mode                     # Show current memory mode
    ares honcho mode [hybrid|honcho|local]  # Set memory mode
    ares honcho tokens                   # Show token budget settings
    ares honcho tokens --context N       # Set session.context() token cap
    ares honcho tokens --dialectic N     # Set dialectic result char cap
    ares honcho identity                 # Show AI peer identity representation
    ares honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    ares honcho migrate                  # Step-by-step migration guide: OpenClaw native → Ares + Honcho
    ares version             Show version
    ares update              Update to latest version
    ares uninstall           Uninstall Ares Agent
    ares acp                 Run as an ACP server for editor integration
    ares sessions browse     Interactive session picker with search

    ares claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: ares_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``ares_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``ares update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``ares_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, ares crashes on import and the user can't run
# ``ares update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import ares_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ARES_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (ares tools, ares setup, ares model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'ares {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any ares module import.
#
# Many modules cache ARES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ARES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.ares/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ARES_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors ares_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ARES_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.ares/profiles/coder or
    # /opt/data/profiles/coder).  If ARES_HOME points to the ares root
    # instead (e.g. systemd hardcodes ARES_HOME=/root/.ares), we must
    # still read active_profile — the user may have switched profiles via
    # `ares profile use` and the gateway should honour that choice.
    # See issue #22502.
    ares_home_env = os.environ.get("ARES_HOME", "")
    if profile_name is None and ares_home_env:
        if Path(ares_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the ares root
    if profile_name is None:
        try:
            from ares_constants import get_default_ares_root

            active_path = get_default_ares_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ARES_HOME
    if profile_name is not None:
        try:
            from ares_cli.profiles import resolve_profile_env

            ares_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent ares from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ARES_HOME"] = ares_home
...
# Load .env from ~/.ares/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from ares_cli.config import get_ares_home
from ares_cli.env_loader import load_ares_dotenv

load_ares_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ARES_REDACT_SECRETS env
# var BEFORE ares_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "ARES_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_ares_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["ARES_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `ares` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from ares_logging import setup_logging as _setup_logging
...
try:
    from ares_cli.config import load_config as _load_config_early
    from ares_constants import apply_ipv4_preference as _apply_ipv4
...
from ares_cli import __version__, __release_date__
from ares_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from ares_cli.config import get_env_path, get_ares_home, load_config
    from ares_cli.auth import get_auth_status

    # Determine whether Ares itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from ares_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_ares_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_ares_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Ares has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Ares to use their tokens.
    if _has_ares_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, ares_bin
        cli_args: the original CLI arguments (everything after 'ares')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    ares_bin = container_info["ares_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo ares {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo ares {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @ares/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@ares" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``ARES_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ARES_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    ares_home = os.environ.get("ARES_HOME") or str(Path.home() / ".ares")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ARES_HOME": ares_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(ares_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(ares_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if ares_cli_dir is None:
        ares_cli_dir = Path(__file__).parent
    bundled = ares_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ARES_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ARES_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from ares_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ARES_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ARES_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ARES_TUI_DIR (e.g. `unset ARES_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("ARES_QUIET"):
    ...
    if tui_dev:
        # Keep the local @ares/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @ares/ink resolves through
        # packages/ares-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "ares-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from ares_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="ares-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    ares_home = os.environ.get("ARES_HOME") or str(Path.home() / ".ares")
    env.setdefault("ARES_HOME", ares_home)
    # The Ares fork still reuses the Hermes TUI/gateway package, whose
    # runtime env contract is HERMES_*. Mirror Ares values so direct
    # `ares --tui` and dashboard PTY launches use the Ares home/config while
    # preserving the ARES_* names tests and wrappers already assert.
    env.setdefault("HERMES_HOME", env["ARES_HOME"])

    def set_tui_env(name: str, value: str) -> None:
        env[f"ARES_TUI_{name}"] = value
        env[f"HERMES_TUI_{name}"] = value

    env["ARES_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["HERMES_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ARES_PYTHON_SRC_ROOT"] = os.environ.get(
        "ARES_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env["HERMES_PYTHON_SRC_ROOT"] = os.environ.get(
        "HERMES_PYTHON_SRC_ROOT", env["ARES_PYTHON_SRC_ROOT"]
    )
    env.setdefault("ARES_PYTHON", sys.executable)
    env.setdefault("HERMES_PYTHON", env["ARES_PYTHON"])
    env.setdefault("ARES_CWD", os.getcwd())
    env.setdefault("HERMES_CWD", env["ARES_CWD"])
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ARES_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ARES_MODEL"] = model
        env["ARES_INFERENCE_MODEL"] = model
        env["HERMES_MODEL"] = model
        env["HERMES_INFERENCE_MODEL"] = model
    if provider:
        env["ARES_TUI_PROVIDER"] = provider
        env["HERMES_TUI_PROVIDER"] = provider
        env["ARES_INFERENCE_PROVIDER"] = provider
        env["HERMES_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        set_tui_env("TOOLSETS", ",".join(tui_toolsets))
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                set_tui_env("SKILLS", ",".join(flattened))
        else:
            value = str(skills).strip()
            if value:
                set_tui_env("SKILLS", value)
    if query:
        set_tui_env("QUERY", query)
    if image:
        set_tui_env("IMAGE", image)
    if checkpoints:
        set_tui_env("CHECKPOINTS", "1")
    if pass_session_id:
        set_tui_env("PASS_SESSION_ID", "1")
    if max_turns is not None:
        set_tui_env("MAX_TURNS", str(max_turns))
    if verbose:
        set_tui_env("TOOL_PROGRESS", "verbose")
    elif quiet:
        set_tui_env("TOOL_PROGRESS", "off")
    if accept_hooks:
        env["ARES_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        set_tui_env("RESUME", resume_session_id)
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ARES_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``ares kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``ares kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ARES_KANBAN_BOARD"):
        return
    try:
        from ares_cli.kanban_db import get_current_board

        os.environ["ARES_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("ARES_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'ares sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Ares isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  ares setup")
        print()

        from ares_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'ares setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from ares_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ARES_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.ares/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ARES_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ARES_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ARES_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from ares_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from ares_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from ares_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Ares?")
        print()
    ...
    # has no creds.json.  Every subsequent `ares gateway` then paid a 30s
    ...
    session_dir = get_ares_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: ares gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `ares gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  ares gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Ares Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  ares gateway")
        ...
        print()
        print("  Or install as a service: ares gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'ares whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from ares_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from ares_cli.config import stamp_install_method
    from ares_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Ares post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``ares model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from ares_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from ares_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from ares_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ARES_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'ares model' for "
                "available providers, or run 'ares doctor' to diagnose config "
                "issues."
            )
    ...
    from ares_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from ares_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.ares/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.ares/.env if the active provider is not 'custom'.
    ...
    """
    from ares_cli.config import get_env_value, save_env_value, load_config
...
#
# Ares uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `ares model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `ares model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from ares_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Ares only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``ares model`` flow, then route aux tasks to them here.
    """
    from ares_cli.config import load_config
    from ares_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from ares_cli.auth import _prompt_model_selection
    from ares_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from ares_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from ares_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.ares/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `ares setup` from scratch. OpenRouter
    ...
    from ares_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.ares/.env by hand.
    ...
    from ares_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from ares_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from ares_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from ares_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `ares auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from ares_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model=""):
    ...
    from ares_cli.models import _PROVIDER_MODELS
    ...
    # live only in the pool (e.g. after ``ares auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from ares_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from ares_cli.auth import _save_model_choice, deactivate_provider
    from ares_cli.config import get_env_value, load_config, save_config
    ...
    from ares_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `ares model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from ares_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Ares URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from ares_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from ares_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from ares_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Ares will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Ares will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from ares_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from ares_cli.config import save_env_value, load_config, save_config
    from ares_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from ares_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from ares_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Ares turns to `copilot --acp`.")
    print("  Ares currently starts its own ACP subprocess for each request.")
    print("  Ares uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ARES_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``ares setup`` / ``ares model``.
    ...
    recover from a malformed paste without editing ``~/.ares/.env`` by hand.
    ...
    from ares_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from ares_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `ares setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so ares knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Ares typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Ares, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from ares_cli.auth import AuthError
        from ares_cli.models import fetch_lmstudio_models
        ...
        from ares_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from ares_constants import display_ares_home as _dhh_fn

            print(
                f"    Ares will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from ares_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'ares model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Ares CLI with a provider."""
    from ares_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from ares_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from ares_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from ares_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from ares_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from ares_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``ares slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: ares slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `ares slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from ares_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from ares_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from ares_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from ares_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from ares_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from ares_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from ares_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Ares home directory to a zip file."""
    if getattr(args, "quick", False):
        from ares_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from ares_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Ares backup from a zip file."""
    from ares_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Ares Agent v{__version__} ({__release_date__})")
    ...
    try:
        from ares_cli.banner import check_for_updates
        from ares_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Ares Agent."""
    _require_tty("uninstall")
    from ares_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``ares update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``ares_cli/web_dist/`` (per vite.config.ts
    outDir: "../ares_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "ares_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``ares update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``ares web``).
    ...
    # build path usable on a stock `py -m ares_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (ares web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``ares dashboard`` processes other than ourselves.

    ``ares dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``ares update`` replaces files on disk, the running
    ...
    patterns = [
        "ares dashboard",
        "ares_cli.main dashboard",
        "ares_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "ares.*dashboard"`) keeps us consistent
            # with `ares_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `ares update`.
    ...
    print("  Preview now:  ares curator run --dry-run")
    print("  Pause it:     ares curator pause")
    print(
        "  Docs:         https://ares-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``ares update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``ares update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: ares curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``ares dashboard`` processes.

    Called at the end of ``ares update`` (default ``reason``) and also
    from ``ares dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    ares dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Ares Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/ares-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="ares-update-")
        zip_path = os.path.join(tmp_dir, f"ares-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to ares-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"ares-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "ares-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Ares behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes ares completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Ares couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Ares couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Ares behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `ares update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/ares-agent.git",
    "git@github.com:NousResearch/ares-agent.git",
    "https://github.com/NousResearch/ares-agent",
    "git@github.com:NousResearch/ares-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/ares-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_ares_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_ares_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Ares repository.")
        print("  This means you may miss updates from NousResearch/ares-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/ares-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/ares-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``ares update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from ares_constants import get_default_ares_root

    default_home = get_default_ares_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``ares update`` is still progressing even if pip/uv itself is silent.
    """
...
def _ares_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "ares.exe",
        scripts_dir / "ares-gateway.exe",
    ]


def _quarantine_running_ares_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``ares.exe``.
    ...
    when ``ares update`` runs, ``ares.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``ares.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next ares invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _ares_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_ares_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``ares.exe.old.*`` left by prior updates.

    Called early on every ares invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``ares update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `ares update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``ares.exe`` / ``ares-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_ares_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_ares_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.ares/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``ares update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.ares/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``ares update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``ares update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.ares/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``ares update --gateway``) the update is already
    ...
    try:
        ...
        # ares_cli.config.get_ares_home to simulate setup failure.
        from ares_cli.config import get_ares_home as _get_ares_home

        logs_dir = _get_ares_home() / "logs"
...
def _cmd_update_check():
    """Implement ``ares update --check``: fetch and report without installing."""
    from ares_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from ares_cli.config import recommended_update_command
        from ares_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``ares update`` without requiring a reinstall.  The
    ...
    ``ares`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/ares.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v ares'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/ares, code at /usr/local/lib/ares-agent).
    fhs_link = Path("/usr/local/bin/ares")
    ...
    path_comment = (
        "# Ares Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ARES_HOME before running the update.
    ...
    ARES_HOME directories.  The ``--backup`` flag on ``ares update``
    ...
    try:
        from ares_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_ares_home so the user sees ~/.ares/...
    try:
        from ares_constants import get_ares_home, display_ares_home

        home = get_ares_home()
        try:
            display_path = f"{display_ares_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  ares import {out_path}")
...
def cmd_update(args):
    """Update Ares Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from ares_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Ares Agent")
        return
...
def _cmd_update_pip(args):
    """Update Ares via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "ares-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "ares-agent"]
    ...
    print("✓ Update complete! Restart ares to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_ares_home added to ares_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload ares_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_ares_home() added since the last release.
        try:
            import importlib
            import ares_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ARES_HOME so
        # it is not affected by sync_skills()'s module-level ARES_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ARES_HOME env var points at the default or a named profile.
        try:
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: ares config migrate")
            else:
                print()
                print("Skipped. Run 'ares config migrate' later to configure.")
        else:
        ...
        # check `ares curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``ares update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from ares_cli.tools_config import install_cua_driver
        ...
        # When running as ``ares update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_ares_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all ares-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. ares-gateway.service or ares-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: ares gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: ares -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Ares gateway unit files are still installed.
        # When both ares.service (from a pre-rename install) and the
        # current ares-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `ares update` surfaces the issue until the user migrates.
        try:
            from ares_cli.gateway import (
                has_legacy_ares_units,
                _find_legacy_ares_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_ares_units():
                print()
                print("⚠ Legacy Ares gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_ares_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (ares.service) fight the current")
                print("  ares-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    ares gateway migrate-legacy")
                print()
        ...
        print("  ares model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``ares -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from ares_constants import display_ares_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `ares profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_ares_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → ares -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.ares)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  ares profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     ares -p {name} chat")
                else:
        ...
        from ares_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from ares_constants import get_ares_home as _hh
        ...
        from ares_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `ares profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from ares_cli.profiles import profile_exists
        ...
        from ares_cli.profiles import rename_profile
        ...
        from ares_cli.profiles import export_profile
        ...
        from ares_cli.profiles import import_profile
        ...
        import tempfile
        from ares_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="ares_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  ares -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      ares -p {plan.manifest.name} chat")
        ...
        from ares_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `ares profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"ares -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from ares_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("ares_requires"):
            print(f"Requires:     Ares {data['ares_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from ares_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.ares_requires:
        print(f"  Requires: Ares {mf.ares_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``ares dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``ares dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No ares dashboard processes running.")
        return 0

    print(f"{len(pids)} ares dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No ares dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `ares update`.
    ...
    if "ARES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ARES_WEB_DIST"])
            if "ARES_WEB_DIST" in os.environ
            else PROJECT_ROOT / "ares_cli" / "web_dist"
        )
    ...
    from ares_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("ARES_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from ares_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Ares log files."""
    from ares_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from ares_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``ares -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``ares_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``ares -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``ares`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for ares CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from ares_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``ares.exe.old.*`` quarantine files left by previous
    # ``ares update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_ares_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from ares_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: ares-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from ares_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://ares-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `ares model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy ares.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Ares gateway unit files "
            "(e.g. ares.service) left over from older installs. Profile "
            "units (ares-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider (default: nous). See `ares proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Ares Agent with an interactive wizard. "
        "Run a specific section: ares setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'ares setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Ares.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ARES_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Ares")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from ares_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Ares CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: ares-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Ares with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set ARES_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Ares Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.ares/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Ares profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.ares/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Ares profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from ares_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.ares/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.ares/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Ares Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `ares "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Ares setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Ares Agent. Use 'ares debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    ares debug share              Upload debug report and print URL
    ares debug share --lines 500  Include more log lines
    ares debug share --expire 30  Keep paste for 30 days
    ares debug share --local      Print report locally (no upload)
    ares debug share --no-redact  Disable upload-time secret redaction
    ares debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'ares debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Ares home directory to a zip file",
        description="Create a zip archive of your entire Ares configuration, "
        "skills, sessions, and data (excludes the ares-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/ares-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.ares/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo ares uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from ares_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Ares backup from a zip file",
        description="Extract a previously created Ares backup into your "
        "Ares home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Ares Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from ares_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.ares/skills/.bundled_manifest) "
            "so future 'ares update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from ares_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from ares_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/ares-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `ares plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from ares_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``ares --help``, ``ares version``, ``ares logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from ares_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from ares_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_ares_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_ares_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_ares_home()}/memories/\n")
        else:
            from ares_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'ares tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # ares tools disable <name...> [--platform cli]
    ...
    # ares tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from ares_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from ares_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `ares computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `ares tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: ares computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: ares computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Ares as an MCP server",
        description=(
            "Manage MCP server connections and run Ares as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'ares mcp add' to connect to a new server, or\n"
            "'ares mcp serve' to expose Ares conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Ares as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `ares mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from ares_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_ares_home() / "sessions"
            ...
            # Launch ares --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from ares_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Ares",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Ares",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.ares/ (by default a "
        "single restore-point archive is written to ~/.ares/backups/ "
        "before apply; restorable with 'ares import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from ares_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Ares Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'ares config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Ares Agent",
        description="Remove Ares Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Ares Agent as an ACP (Agent Client Protocol) server",
        description="Start Ares Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Ares ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Ares provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.ares/node/ "
    ...
    def cmd_acp(args):
        """Launch Ares Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Ares instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `ares update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `ares profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Ares profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Ares Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `ares --tui` via PTY/WebSocket). "
            "Alternatively set ARES_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `ares dashboard` cmdlines and SIGTERM them directly — the same
    # path `ares update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running ares dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running ares dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Ares log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    ares logs                    Show last 50 lines of agent.log
    ares logs -f                 Follow agent.log in real time
    ares logs errors             Show last 50 lines of errors.log
    ares logs gateway -n 100     Show last 100 lines of gateway.log
    ares logs --level WARNING    Only show WARNING and above
    ares logs --session abc123   Filter by session ID
    ares logs --component tools  Only show tool-related lines
    ares logs --since 1h         Lines from the last hour
    ares logs --since 30m -f     Follow, starting from 30 min ago
    ares logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``ares -c Pokemon Agent Dev`` → ``ares -c 'Pokemon Agent Dev'``
    ...
    from ares_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'ares -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (ares hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from ares_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from ares_cli.oneshot import run_oneshot
