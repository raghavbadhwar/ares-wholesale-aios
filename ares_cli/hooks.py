"""ares hooks — inspect and manage shell-script hooks.

Usage::

    ares hooks list
    ares hooks test <event> [--for-tool X] [--payload-file F]
    ares hooks revoke <command>
    ares hooks doctor

Consent records live under ``~/.ares/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.ares/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``ares hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: ares hooks {list|test|revoke|doctor}")
        print("Run 'ares hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.ares/config.yaml.")
        print("See `ares hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `ares hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `ares hooks test` and `ares hooks doctor`
...
def _cmd_test(args) -> None:
    from ares_cli.config import load_config
    from ares_cli.plugins import VALID_HOOKS
    from agent import shell_hooks
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Ares wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `ares hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `ares hooks doctor` would execute
