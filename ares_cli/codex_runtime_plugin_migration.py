"""Migrate Ares' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Ares' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Ares mcp_servers.<n>.command/args/env  → codex stdio transport
  Ares mcp_servers.<n>.url/headers       → codex streamable_http transport
  Ares mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Ares mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Ares-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Ares' own
...
MIGRATION_MARKER = (
    "# managed by ares-agent — `ares codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end ares-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Ares config.")
...
# Ares keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Ares' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, ares_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Ares MCP server config to the codex inline-table dict
    ...
    if not isinstance(ares_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(ares_cfg.get("command"))
    has_url = bool(ares_cfg.get("url"))
    ...
    for key in ares_cfg:
        ...
        elif key not in _KNOWN_ARES_KEYS:
            skipped.append(f"{key} (unknown Ares key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ARES_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Ares)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Ares' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Ares' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Ares
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="ares-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ARES_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed ares-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.ares`` symlink that happens to live under ``/private/var/folders``
...
def _build_ares_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Ares' own
    ...
    so a ares installed under /opt/, /usr/local/, or a venv all work.
    ARES_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ARES_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_ares_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ARES_HOME its launcher sets
    ...
    # — that would override the launcher's ARES_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ARES_HOME", tmp_path) would
    ...
    ares_home = os.environ.get("ARES_HOME") or ""
    if ares_home and _looks_like_test_tempdir(ares_home):
        ares_home = ""
    if ares_home:
        env["ARES_HOME"] = ares_home
    # PYTHONPATH passes through so a worktree-launched ares finds the
    ...
    env["ARES_QUIET"] = "1"
    env["ARES_REDACT_SECRETS"] = env.get("ARES_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_ares_tools: bool = True,
) -> MigrationReport:
    """Translate Ares mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        ares_config: full ~/.ares/config.yaml dict
        ...
        expose_ares_tools: when True (default), register Ares' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Ares for tools
    ...
    ares_servers = (ares_config or {}).get("mcp_servers") or {}
    if not isinstance(ares_servers, dict):
        report.errors.append(
            "mcp_servers in Ares config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in ares_servers.items():
    ...
    # Inject Ares' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Ares for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/ares_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_ares_tools:
        translated["ares-tools"] = _build_ares_tools_mcp_entry()
        if "ares-tools" not in report.migrated:
            report.migrated.append("ares-tools")
