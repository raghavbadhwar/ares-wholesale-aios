"""ares webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    ares webhook subscribe <name> [options]
    ares webhook list
    ares webhook remove <name>
    ares webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.ares/webhook_subscriptions.json and are
...
def _ares_home() -> Path:
    ...
    return get_ares_home()


def _subscriptions_path() -> Path:
    return _ares_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: ares gateway run
"""
...
def webhook_command(args):
    """Entry point for 'ares webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: ares webhook {subscribe|list|remove|test}")
        print("Run 'ares webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (ares gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: ares webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from ares webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (ares gateway run)")
