"""Profile distributions — shareable, packaged Ares profiles via git.

A distribution is a Ares profile published as a git repository (or
...
* ``ares profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``ares skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``ares profile``, not a parallel tree):

    ares profile install <source> [--name N] [--alias] [--force] [--yes]
    ares profile update  <name>  [--force-config] [--yes]
    ares profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "ares-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            ares_requires=str(data.get("ares_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.ares_requires:
            out["ares_requires"] = self.ares_requires
        if self.author:
...
def check_ares_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Ares {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Ares distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``ares profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Ares profile distribution."
            )
...
def plan_install(
    ...
    from ares_cli import __version__ as ares_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Ares distribution."
        )

    # Version check up-front so we fail fast
    check_ares_requires(manifest.ares_requires, ares_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.ares).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="ares_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `ares profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`ares profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="ares_dist_update_") as tmp:
        plan = plan_install(
