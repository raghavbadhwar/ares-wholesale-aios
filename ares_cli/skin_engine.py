"""Ares CLI skin/theme engine.

A data-driven skin system that lets users customize the CLI's visual appearance.
Skins are defined as YAML files in ~/.ares/skins/ or as built-in presets.
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Ares Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Ares "       # Response box header label
...
.. code-block:: python
    ...
    from ares_cli.skin_engine import get_active_skin, list_skins, set_active_skin
    ...
    set_active_skin("mytheme")            # Switch to user skin from ~/.ares/skins/

BUILT-IN SKINS
==============

- ``default`` — Classic Ares gold/kawaii (the current look)
...
Drop a YAML file in ``~/.ares/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces ARES_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces ARES_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Ares — gold and kawaii",
        "colors": {
