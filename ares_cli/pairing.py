...
Usage:
    ares pairing list              # Show all pending + approved users
    ares pairing approve <platform> <code>  # Approve a pairing code
    ares pairing revoke <platform> <user_id> # Revoke user access
    ares pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle ares pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: ares pairing {list|approve|revoke|clear-pending}")
        print("Run 'ares pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.ares/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'ares pairing list' to see pending codes.\n")
