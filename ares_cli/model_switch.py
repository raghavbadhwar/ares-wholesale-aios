...
- ``ares_cli.providers``        -- canonical provider identity + overlays
- ``ares_cli.model_normalize``  -- per-provider name formatting
...
from ares_cli.model_normalize import (
...
_ARES_MODEL_WARNING = (
    "Nous Research Ares 3 & 4 models are NOT agentic and are not designed "
    "for use with Ares Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Ares 3 / Ares 4 chat families.
# The previous substring check (`"ares" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``ares-brain:qwen3-14b-ctx16k`` that just
# happen to carry "ares" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Ares-3-Llama-3.1-70B, ares-4-405b, openrouter/ares3:70b
# Negative examples it must NOT match:
#   ares-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ARES_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])ares[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_ares_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Ares 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ARES_NON_AGENTIC_RE.search(model_name))


def _check_ares_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Ares 3/4 chat model."""
    if is_nous_ares_non_agentic(model_name):
        return _ARES_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``ares config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from ares_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'ares model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'ares doctor' — config issues detected:"
        ...
        if not new_model:
            if pdef.base_url:
                from ares_cli.runtime_provider import _auto_detect_local_model
    ...
    # Mirrors the same logic in ares_cli.runtime_provider.resolve_runtime_provider;
    ...
    ares_warn = _check_ares_model_warning(new_model)
    if ares_warn:
        warnings.append(ares_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from ares_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from ares_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by ares provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://ares-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Ares release. Falls back to the in-repo
    ...
    # --- 1. Check Ares-mapped providers ---
    for ares_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(ares_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ARES_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from ares_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and ares_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Ares release.
        model_ids = curated.get(ares_id, [])
        if ares_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(ares_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Ares-only providers (nous, openai-codex, copilot, opencode-go) ---
    from ares_cli.providers import ARES_OVERLAYS
    from ares_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Ares provider ID.
    # ARES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Ares IDs ("copilot").
    _mdev_to_ares = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ARES_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Ares slug — e.g. "github-copilot" → "copilot"
        ares_slug = _mdev_to_ares.get(pid, pid)
        if ares_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(ares_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, ares_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or ares_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(ares_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", ares_slug, exc)
        ...
        # configured.
        if not has_creds and ares_slug == "anthropic":
            try:
                ...
                if (ares_creds and ares_creds.get("accessToken")) or \
        ...
        if ares_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(ares_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(ares_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(ares_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Ares slug, fall back to overlay key
            model_ids = curated.get(ares_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if ares_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(ares_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": ares_slug,
            "name": get_label(ares_slug),
            "is_current": ares_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "ares",
        ...
        seen_slugs.add(ares_slug.lower())
        _record_builtin_endpoint(ares_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ARES_OVERLAYS (keeps /model in sync
    # with `ares model`).
    try:
        from ares_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Ares's canonical write key (matches
            ...
            # Ares writes ``models:`` as a dict keyed by model id
            # (see ares_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Ares's own writer uses; a hyphen variant is accepted
            ...
            # active model. Ares's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, ares_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`ares_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from ares_cli.models import fetch_openrouter_models
