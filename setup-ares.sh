...
# 5. Symlinks the 'ares' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Ares Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/ares-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/ares-uv-installer.$$.sh")"
...
# ============================================================================
# PATH setup — symlink ares into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up ares command..."

ARES_BIN="$SCRIPT_DIR/venv/bin/ares"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ARES_BIN" "$COMMAND_LINK_DIR/ares"
echo -e "${GREEN}✓${NC} Symlinked ares → $COMMAND_LINK_DISPLAY_DIR/ares"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Ares Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.ares/skills/
# ============================================================================

ARES_SKILLS_DIR="${ARES_HOME:-$HOME/.ares}/skills"
mkdir -p "$ARES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.ares/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ARES_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  ares status        # Check configuration"
if is_termux; then
    echo "  ares gateway       # Run gateway in foreground"
else
    echo "  ares gateway install # Install gateway service (messaging + cron)"
fi
echo "  ares cron list     # View scheduled jobs"
echo "  ares doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m ares_cli.main setup
fi
